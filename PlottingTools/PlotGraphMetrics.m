...
%          imrotateticklabel() --
%          PlotRegionColouredBars() --
%          GroupRegions() --
%
% USAGE:
%{
        Connectivity.WhichMatrix = 'RM_AC';
        PlotGraphMetrics(Connectivity)
%}
%
...
function PlotGraphMetrics(Connectivity)
 
%% Make sure the "delay" matrix will also be a distance matrix
  if nargin>1,
    if isfield(options,'invel'),
      warning(strcat('PlottingTools:',mfilename,':Resetting_invel'), 'Resetting the invel you provided to 1.0, so that delay is a distance matrix');
    end
  end
  options.invel = 1.0;

  ThisMatrix = Connectivity.WhichMatrix;

%% Get the connectivity matrix...
  Connectivity = GetConnectivity(Connectivity);
  %THE SENSE OF OUR MATRICES IS REVERSED FROM THAT EXPECTED BY THE GRAPH METRIC ALGORITHMS... 
  %(Algorithms expect n1 -> n2 = x(1,2); Matrices are n1 -> n2 = x(2,1))
  weights = Connectivity.weights.';
  delay   = Connectivity.delay.';
  NodeStr = Connectivity.NodeStr;
...
%%%keyboard
  while any(In==0) || any(Out==0),
    if any(In==0),
      msg = 'The network appears to be disconnected: Our metrics require a path to and from every node, removing nodes that can''t be reached to avoid Inf';
      warning(strcat('PlottingTools:',mfilename,':NetworkDisconnected'), msg);
    ...
    if any(Out==0),
      msg = 'The network appears to be disconnected: Our metrics require a path to and from every node, removing nodes that can''t be escaped from to avoid Inf';
      warning(strcat('PlottingTools:',mfilename,':NetworkDisconnected'), msg);
