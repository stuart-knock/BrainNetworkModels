...
%
% ARGUMENTS:
%           doi       -- structure containing the original data from which
%                        dnn* was calculated. Must be in standard doi-type
%                        EEG structure.
...
%{
   ...
   %Put it in doi structure format
    ...
    segNum = 1:doi.Epochs;
    figureNum = 666;
    inspectdoi(doi, figureNum, segNum, doi.NodeStr);
%}
%
% MODIFICATION HISTORY:
%     AJL (Nov-2006) -- Original
%     SAK(24-11-2006) -- Slight modifications: generalised for all dnn*; 
%                        colourmap; ranges; forward & backward; comments.
...
%%%NEED TO CONSIDER WINDOWING FOR THIS...
  fprintf(1,'Precalculating for %d channels, currently on channel:     ', length(datastr));
  for chan = 1:length(datastr),
    fprintf(1,'\b\b\b\b%4d', chan); %channel counter...
  ...
  fprintf(1,'\n'); %Close off the channel counter...
  f = 0:(doi.SampleRate/2 -1);
%keyboard
%% Allow forward and backward stepping over epochs and channels
  direction = 1;
  chan = 1;
